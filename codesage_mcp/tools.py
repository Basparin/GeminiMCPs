...
def search_codebase_tool(
    codebase_path: str, pattern: str, file_types: list[str] = None, exclude_patterns: list[str] = None
) -> dict:
...
def summarize_code_section_tool(
    ...
    try:
        if function_name or class_name:
            ...
            if not found_node:
                return {"error": {"code": "NOT_FOUND", "message": f"Function/Class {function_name or class_name} not found in {file_path}."}}
...
def get_file_structure_tool(codebase_path: str, file_path: str) -> dict:
    ...
    return {"message": f"File structure for {file_path} in {codebase_path}:", "structure": file_structure}
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if not (node.body and isinstance(node.body[0], ast.Expr) and isinstance(node.body[0].value, ast.Constant) and isinstance(node.body[0].value.value, str)):
                    undocumented_functions.append({"name": node.name, "line_number": node.lineno})

        if undocumented_functions:
            return {
                "message": f"Found {len(undocumented_functions)} undocumented functions in {file_path}.",
...
def count_lines_of_code_tool() -> dict:
    ...
    if current_codebase_path not in codebase_manager.indexed_codebases:
        return {"error": {"code": "NOT_INDEXED", "message": f"Codebase at {current_codebase_path} has not been indexed. Please index it first."}}
...
def configure_api_key_tool(llm_provider: str, api_key: str) -> dict:
    ...
    if not env_var_name:
        return {"error": {"code": "INVALID_PROVIDER", "message": f"Unsupported LLM provider: {llm_provider}. Supported providers are: {', '.join(env_var_map.keys())}."}}
    ...
    return {"message": f"API key for {llm_provider} updated successfully. A server restart may be required for changes to take full effect."}
...
def get_dependencies_overview_tool() -> dict:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        ...
                        if os.path.exists(os.path.join(current_codebase_path, module_name)) or \
                           os.path.exists(os.path.join(current_codebase_path, module_name + ".py")) or \
                           os.path.exists(os.path.join(current_codebase_path, module_name, "__init__.py")):
    ...
    return {
        ...
        "total_python_files_analyzed": len(internal_dependencies) + len(external_dependencies), # This is not accurate, need to fix
        "total_internal_dependencies": sum(len(deps) for deps in internal_dependencies.values()),
        "total_external_dependencies": sum(len(deps) for deps in external_dependencies.values()),
        "unique_external_modules": sorted(list(all_external_modules)),
        "internal_dependencies_by_file": {k: sorted(list(v)) for k, v in internal_dependencies.items()},
        "external_dependencies_by_file": {k: sorted(list(v)) for k, v in external_dependencies.items()},
    }