...
import asyncio
import logging
from codesage_mcp.core.utils import _count_todo_fixme_comments, safe_read_file  # New import
import json  # New import
from codesage_mcp.core.exceptions import LLMAPIKeyError, LLMResolutionError, LLMProviderError
...
class LLMAnalysisManager:
    ...
    def _get_llm_response(
        ...
        async def _call_openrouter():
            if not self.openrouter_client:
                raise LLMAPIKeyError("OPENROUTER_API_KEY not configured.", llm_provider="openrouter", env_var_name="OPENROUTER_API_KEY")
        ...
        async def _call_groq():
            if not self.groq_client:
                raise LLMAPIKeyError("GROQ_API_KEY not configured.", llm_provider="groq", env_var_name="GROQ_API_KEY")
        ...
        async def _call_google():
            if not self.google_ai_client:
                raise LLMAPIKeyError("GOOGLE_API_KEY not configured.", llm_provider="google", env_var_name="GOOGLE_API_KEY")
        ...
        try:
            ...
            if llm_model.startswith("openrouter/"):
                ...
                return None, LLMProviderError(f"LLM model '{llm_model}' not supported yet.", llm_provider=llm_model).message
    ...
    def resolve_todo_fixme(self, file_path: str, line_number: int = None) -> dict:
        ...
        try:
            ...
            if error_message:
                raise LLMResolutionError(f"LLM resolution failed: {error_message}", original_error=error_message)
