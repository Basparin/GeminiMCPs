...
from codesage_mcp.core.error_reporting import error_reporter
from codesage_mcp.config.config import get_optional_env_var


# Alert Management Module
class AlertManager:
    """Handles alerting and notification for regression detection."""
    ...
    async def send_alerts(self, report: 'RegressionReport'):
...
class GitHubIntegration:
    """Handles GitHub issue creation for regressions."""

    def __init__(self, config: 'RegressionConfig'):
        self.config = config
        self.logger = get_logger("github_integration")

    async def create_issues(self, report: 'RegressionReport'):
    ...
    def _format_github_issue_body(self, regression: 'RegressionResult', report: 'RegressionReport') -> str:
        ...
        p_value_str = f"{regression.p_value:.4f}" if regression.p_value is not None else "N/A"
...
class BaselineManager:
    ...
    def __init__(self, config: 'RegressionConfig'):
        self.config = config
        self.logger = get_logger("baseline_manager")
...
class RegressionDetector:
    ...
    def __init__(self, config: Optional[RegressionConfig] = None):
        ...
        # Alert cooldown tracking (for backward compatibility)
        self.last_alert_times: Dict[str, float] = {}
    ...
    def detect_regressions(self, current_results: Dict[str, Any], baseline_results: Optional[Dict[str, Any]] = None) -> RegressionReport:
        ...
        if baseline_results is None:
            baseline_results = self.baseline_manager.load_baseline_results()
    ...
    def _check_regression_criteria(self, metric_name: str, percentage_change: float, is_significant: bool) -> Tuple[bool, str]:
        ...
        # Only flag as regression if it's a degradation (worse performance)
        criteria = {
            "latency": (percentage_change > self.config.latency_regression_threshold, "high"),
            "response_time": (percentage_change > self.config.latency_regression_threshold, "high"),
            "throughput": (percentage_change < -self.config.throughput_regression_threshold, "high"),
            "memory": (percentage_change > self.config.memory_regression_threshold, "medium"),
            "error_rate": (percentage_change > self.config.error_rate_regression_threshold, "critical"),
            "cache_hit_rate": (percentage_change < -self.config.cache_hit_regression_threshold, "medium"),
        }
        ...
        for pattern, (condition, severity) in criteria.items():
            if pattern.lower() in metric_name.lower():
                ...
                return False, "none"  # No regression if condition not met

        # Default: check for significant degradation in any metric
        # For unknown metrics, assume higher values are worse (like latency)
        if abs(percentage_change) > 5.0 and is_significant and percentage_change > 0:
            return True, "low"
    ...
    async def _handle_regressions_async(self, report: RegressionReport):
        ...
        try:
            ...
            if self.config.enable_alerting:
                await self.alert_manager.send_alerts(report)
    ...
    async def _send_alerts(self, report: RegressionReport):
        """Send alerts for detected regressions (delegate to AlertManager)."""
        await self.alert_manager.send_alerts(report)

    async def _create_github_issues(self, report: RegressionReport):
        """Create GitHub issues for detected regressions (delegate to GitHubIntegration)."""
        await self.github_integration.create_issues(report)

    def _format_alert_message(self, report: RegressionReport) -> str:
        """Format regression report for alerts (delegate to AlertManager)."""
        return self.alert_manager._format_alert_message(report)

    def _format_github_issue_body(self, regression: RegressionResult, report: RegressionReport) -> str:
        """Format regression details for GitHub issue (delegate to GitHubIntegration)."""
        return self.github_integration._format_github_issue_body(regression, report)

    def _load_baseline_results(self) -> Dict[str, Any]:
        """Load baseline results from file."""
        return self.baseline_manager.load_baseline_results()

    def save_baseline_results(self, results: Dict[str, Any]):
        """Save results as new baseline."""
        self.baseline_manager.save_baseline_results(results)
