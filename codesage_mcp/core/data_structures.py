"""
Data Structures Module for CodeSage MCP Server.

This module provides core data structures for representing code elements,
relationships, and graph-based code analysis. These structures are designed
to support efficient code analysis, search, and incremental updates.
...
Classes:
    ...
    CodeGraph: Main graph structure managing code elements and relationships
    ASTGenerator: Generates code models from Python source code using AST
"""
...
class CodeNode:
    ...
    id: str
    type: str  # Changed from node_type to type to match test expectations
    name: str
    ...
    def __eq__(self, other):
        """Check equality based on key attributes, ignoring timestamps."""
...
class Relationship:
    ...
    source: str  # Changed from source_id to source to match test expectations
    target: str  # Changed from target_id to target to match test expectations
    type: str    # Changed from relationship_type to type to match test expectations
    layer: LayerType = field(default=LayerType.SEMANTIC)
    metadata: Dict[str, Any] = field(default_factory=dict)
    weight: float = 1.0
    created_at: float = field(default_factory=time.time)
    ...
    def is_valid(self) -> bool:
        ...
        return bool(self.source and self.target and self.type)

    def reverse(self) -> 'Relationship':
        """Reverse the relationship."""
        reversed_type = self.type
        if self.type == "calls":
            reversed_type = "called_by"
        elif self.type == "inherits":
            reversed_type = "inherited_by"
...
class CodeGraph:
    ...
    def get_neighbors(self, node_id: str) -> List[str]:
        """Get neighbors of a node (for test compatibility)."""
        with self._lock:
            neighbors = set()
            for layer in self.layers.values():
                relationships = layer.get_node_relationships(node_id)
    ...
    def remove_node(self, node_id: str) -> None:
        """Remove a node and its relationships (for test compatibility)."""
        with self._lock:
            # Remove from all layers
            for layer in self.layers.values():
                layer.remove_node(node_id)
            ...
            if node_id in self.node_files:
                file_path = self.node_files[node_id]
                if file_path in self.file_nodes:
                    self.file_nodes[file_path].discard(node_id)
                    if not self.file_nodes[file_path]:
                        del self.file_nodes[file_path]
                del self.node_files[node_id]

    def detect_cycle(self) -> bool:
        """Detect cycles in the graph (for test compatibility)."""
...
class ASTGenerator:
    ...
    def parse(self, code: str, file_path: str = "<string>") -> CodeGraph:
        """
        Parse source code and generate a code graph.
        ...
        Returns:
            CodeGraph containing the parsed code structure
        ...
        try:
            ...
            nodes = self._generate_nodes_from_ast(tree, file_path, code)
