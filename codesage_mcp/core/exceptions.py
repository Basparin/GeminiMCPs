...
class LLMError(BaseMCPError):
    ...
    def __init__(
        ...
        full_context = {"llm_provider": llm_provider, "model_name": model_name}
        if context:
            full_context.update(context)
        super().__init__(error_code, message, full_context)
...
class LLMAPIKeyError(LLMError):
    """Exception raised when an LLM API key is missing or invalid."""
    ...
    def __init__(
        ...
        full_context = {"env_var_name": env_var_name}
        if context:
            full_context.update(context)
        super().__init__(message, llm_provider=llm_provider, context=full_context)


class LLMResolutionError(LLMError):
    """Exception raised when an LLM fails to provide a satisfactory resolution or response."""
    ...
    def __init__(
        ...
        error_code = "LLM_RESOLUTION_ERROR"
        full_context = {"original_error": original_error}
        if context:
            full_context.update(context)
        super().__init__(message, llm_provider=llm_provider, model_name=model_name, context=full_context)


class LLMProviderError(LLMError):
    """Exception raised for issues related to an unsupported or misconfigured LLM provider."""
    ...
    def __init__(
        ...
        super().__init__(message, llm_provider=llm_provider, context=context)


class InvalidPatternError(BaseMCPError):
    """Exception raised for invalid search patterns."""
    ...
    def __init__(
        ...
        full_context = {"pattern": pattern, "original_error": original_error}
        if context:
            full_context.update(context)
        super().__init__(error_code, message, full_context)


class InvalidPathError(BaseMCPError):
...
class EmbeddingError(BaseMCPError):
    ...
    def __init__(
        ...
        full_context = {"index": index, "embedding_type": embedding_type}
        if context:
            full_context.update(context)
        super().__init__(error_code, message, full_context)