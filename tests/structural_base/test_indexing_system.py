...
Tests are organized following the project's coding standards and naming conventions.
"""
...
@pytest.fixture
def indexing_manager(temp_directory):
    """Create an IndexingManager instance with temporary directory."""
    return IndexingManager(index_dir_name=temp_directory)
...
@pytest.fixture
def sample_codebase(temp_directory):
    """Create a sample codebase structure for testing."""
    codebase_dir = Path(temp_directory) / "test_codebase"
    codebase_dir.mkdir()

    # Create sample Python files
    (codebase_dir / "main.py").write_text("print('Hello, World!')\nimport utils")
    (codebase_dir / "utils.py").write_text("def helper():\n    return 'helper'")
...
class TestIndexingManager:
    ...
    def test_initialization(self, temp_directory):
        """Test IndexingManager initialization with proper directory setup."""
        manager = IndexingManager(index_dir_name=temp_directory)

        assert manager.index_dir_name == temp_directory
        assert manager.index_dir == Path(temp_directory)
        assert manager.index_file == Path(temp_directory) / "codebase_index.json"
        assert manager.faiss_index_file == Path(temp_directory) / "codebase_index.faiss"
...
class TestIndexingManagerOptimizationAdvanced:
    """Test cases for advanced index optimization functionality."""
...
class TestIndexingManagerGitignorePatterns:
    """Test cases for .gitignore pattern matching functionality."""

    def test_is_ignored_directory_patterns(self):
        """Test ignoring files based on directory patterns."""
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = IndexingManager()

            # Create test directory structure
            root_path = Path(temp_dir)
            (root_path / "normal_file.py").write_text("print('normal')")
            (root_path / "venv").mkdir()
            (root_path / "venv" / "lib.py").write_text("print('lib')")
            (root_path / "foo").mkdir()
            (root_path / "foo" / "bar.py").write_text("print('bar')")

            patterns = ["venv/", "foo/"]

            # Test directory patterns
            assert manager._is_ignored(root_path / "venv" / "lib.py", patterns, root_path) is True
            assert manager._is_ignored(root_path / "foo" / "bar.py", patterns, root_path) is True
            assert manager._is_ignored(root_path / "normal_file.py", patterns, root_path) is False

    def test_is_ignored_nested_directory_patterns(self):
        """Test ignoring files in nested directories matching patterns."""
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = IndexingManager()

            root_path = Path(temp_dir)
            (root_path / "project").mkdir()
            (root_path / "project" / "venv").mkdir()
            (root_path / "project" / "venv" / "lib.py").write_text("print('lib')")
            (root_path / "project" / "src").mkdir()
            (root_path / "project" / "src" / "main.py").write_text("print('main')")

            patterns = ["venv/"]

            # Test nested directory matching
            assert manager._is_ignored(root_path / "project" / "venv" / "lib.py", patterns, root_path) is True
            assert manager._is_ignored(root_path / "project" / "src" / "main.py", patterns, root_path) is False

    def test_is_ignored_file_name_patterns(self):
        """Test ignoring files based on file name patterns."""
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = IndexingManager()

            root_path = Path(temp_dir)
            (root_path / "test.py").write_text("print('test')")
            (root_path / "main.py").write_text("print('main')")
            (root_path / "utils.py").write_text("print('utils')")

            patterns = ["test.py"]

            # Test file name patterns
            assert manager._is_ignored(root_path / "test.py", patterns, root_path) is True
            assert manager._is_ignored(root_path / "main.py", patterns, root_path) is False
            assert manager._is_ignored(root_path / "utils.py", patterns, root_path) is False


class TestIndexingManagerStatsAndHealth:
    """Test cases for statistics and health monitoring functionality."""

    def test_get_indexing_stats_with_cache(self):
        """Test getting indexing stats when cache is available."""
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = IndexingManager(index_dir_name=temp_dir)
            ...
            mock_cache.get_stats.return_value = {"cache_hits": 5, "cache_misses": 3}
            manager.cache = mock_cache

            # Mock memory manager stats
            mock_memory = MagicMock()
            mock_memory.get_memory_stats.return_value = {"used_mb": 100, "available_mb": 200}
            manager.memory_manager = mock_memory
    ...
    def test_get_indexing_stats_without_cache(self):
        """Test getting indexing stats when cache is disabled."""
        with tempfile.TemporaryDirectory() as temp_dir:
            manager = IndexingManager(index_dir_name=temp_dir)
            ...
            assert stats["index_stats"]["total_vectors"] == 5

    def test_analyze_index_health_healthy_index(self):
        """Test index health analysis for a healthy index."""
        manager = IndexingManager()

        # Create mock healthy index
        mock_index = MagicMock()
        mock_index.ntotal = 100
        mock_index.d = 128
        mock_index.is_trained = True
    ...
    def test_analyze_index_health_no_index(self):
        """Test index health analysis when no index exists."""
        manager = IndexingManager()
        ...
        assert health["healthy"] is False
        assert health["reason"] == "no index"
        # When no index exists, only these keys are present
        assert "total_vectors" not in health

    def test_analyze_index_health_fragmentation(self):
        """Test index health analysis for fragmented index."""
        manager = IndexingManager()

        # Create mock fragmented index
        mock_index = MagicMock()
        ...
        assert "needs_optimization" in health


class TestIndexingManagerOptimization:
    """Test cases for index optimization functionality."""

    def test_optimize_index_comprehensive_no_index(self):
        """Test comprehensive optimization when no index exists."""
        manager = IndexingManager()
        ...
        assert "No index to optimize" in result["message"]

    def test_optimize_index_comprehensive_already_optimized(self):
        """Test comprehensive optimization when index is already optimized."""
        manager = IndexingManager()

        # Create mock optimized index
        mock_index = MagicMock()
        ...
        # Mock health analysis to return no optimization needed
        with patch.object(manager, '_analyze_index_health') as mock_health:
            ...
            assert result["success"] is True
            assert "already optimized" in result["message"]

    def test_compress_index_no_index(self):
        """Test index compression when no index exists."""
        manager = IndexingManager()

        # Mock to simulate no index scenario
        with patch.object(manager, 'faiss_index', None):
            result = manager.compress_index()

            assert result["success"] is False
            assert "No index to compress" in result["message"]

    def test_estimate_index_memory_usage_flat_index(self):
        """Test memory usage estimation for flat index."""
        manager = IndexingManager()

        # Create mock flat index
        mock_index = MagicMock()
        mock_index.ntotal = 100
        mock_index.d = 128
        manager.faiss_index = mock_index

        # Mock PQ attribute to simulate flat index
        mock_index.pq = None
        mock_index.sq = None

        memory_mb = manager._estimate_index_memory_usage()

        # Should use fallback estimation: vectors * dimension * 4 * 1.2 / (1024*1024)
        expected_memory = (100 * 128 * 4 * 1.2) / (1024 * 1024)
        assert memory_mb == expected_memory

    def test_optimize_index_for_memory_within_target(self):
        """Test memory optimization when already within target."""
        manager = IndexingManager()
        ...
        # Mock memory estimation to return value within target
        with patch.object(manager, '_estimate_index_memory_usage') as mock_estimate:
            mock_estimate.return_value = 100  # MB

            result = manager.optimize_index_for_memory(200)  # Target 200MB

            assert result["success"] is True
            assert "already within memory target" in result["message"]
